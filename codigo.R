...
# Crear gráficos interactivos con plotly
p <- plot_ly(x = ~x, y = ~y, z = ~z) %>% add_surface() %>%
  layout(scene = list(xaxis = list(range = c(NA, 100)),
                      yaxis = list(range = c(NA, 100)),
                      zaxis = list(range = c(NA, 100))))
...
graficaDependenciaCopulas2D <- function(myCopulaP, myCopulaN, n = 10000, seed = 123, alpha = 1, pareto_params1 = 1, pareto_params2 = 1) {
  ...
  samplesP <- apply(samplesP, 2, function(x) qexp(x, rate = params1))
  samplesN <- apply(samplesN, 2, function(x) qexp(x, rate = params2))
...
graficaDependenciaCopulas2D2 <- function(myCopulaP, myCopulaN, n = 10000, seed = 123, alpha = 1, pareto_params1 = 1, pareto_params2 = 1) {
  # Cargar librerías necesarias
  library(copula)
  library(ggplot2)
  ...
  p <- ggplot(df, aes(x = X, y = Y, color = Case)) +
    ...
    ggtitle("Dependence between Variables Using Copulas") +
    labs(x = "Variable 1", y = "Variable 2", color = "Dependence Type")+
    coord_cartesian(xlim = c(NA, 100), ylim = c(NA, 100)) # Límites para los ejes
